...
export async function create(input: CreatePurchaseInput) {
  // cálculo monto cuota: base + remainder en la primera cuota (para no perder pesos)
  ...
  const purchase = await repo.createPurchaseRow({
    ...input,
    installment_amount: base, // guardamos base
  });
  ...
  for (let i = 0; i < input.installments; i++) {
    ...
    installmentsToCreate.push({ due_date: due, amount, period_id: periodId });
  }

  const createdInstallments = await instRepo.bulkCreateInstallments(purchase.id, installmentsToCreate);
